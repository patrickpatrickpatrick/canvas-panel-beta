import React__default, { createContext as createContext$1, useContext, useState, useEffect, useMemo, useCallback, useRef, Component } from 'react';
import { parseManifest as parseManifest$1, AnnotationList, Annotation as Annotation$1, Utils } from 'manifesto.js';
...
function createContext(defaultValue) {
    ...
    return [useContext$1, ctx.Provider];
}

function parseManifest(jsonLd, options) {
    return parseManifest$1(jsonLd, options);
}
...
const ManifestInner = ({ children }) => {
    ...
    return functionOrMapChildren(children, useMemo(() => ({
        manifest,
    }), [manifest]));
};
const Manifest = (_a) => {
...
const Bem = ({ children, prefix = '', cssClassMap = {}, }) => {
    return (React__default.createElement(BemContextProvider, { value: useMemo(() => ({
            prefix,
            cssClassMap,
...
function useBemClassName(className) {
    const bemCtx = useBem();
    return useMemo(() => constructBemFromContext(className, Object.assign({}, defaultContext, bemCtx)), [bemCtx, className]);
}
...
const CanvasProvider = ({ startCanvas = 0, sequence = 0, currentCanvas, children, }) => {
    ...
    const manifest = useManifest();
    const setCurrentCanvasFromIdOrIndex = useCallback((value) => {
...
class CustomAnnotationList extends AnnotationList {
    ...
    getResources() {
        ...
        return resources.map((resource) => new Annotation$1(resource, this.options));
    }
}
...
function useAnnotationLists() {
    ...
    const fullAnnotationList = useMemo(() => [...externalAnnotationLists, ...embeddedAnnotationLists], [embeddedAnnotationLists, externalAnnotationLists]);
    return [
        fullAnnotationList,
        {
...
const AnnotationListProvider = ({ annotationList, children }) => {
    return (React__default.createElement(InternalAnnotationListProvider, { value: useMemo(() => ({ annotationList }), [annotationList]) }, children));
};
...
const AnnotationProvider = (_a) => {
    ...
    const parsedList = useMemo(() => annotationList.getResources().map(annotation => ({
        annotationList,
        annotation,
...
class RangeNavigationProvider extends Component {
    constructor() {
        super(...arguments);
        this.state = {
            currentIndex: 0,
...
export { Annotation, AnnotationCanvasRepresentation, AnnotationDetail$1 as AnnotationDetail, AnnotationListProvider$1 as AnnotationListProvider, AnnotationProvider, AnnotationRepresentation, AnnotationSelector, Bem, CanvasNavigation, CanvasProvider$1 as CanvasProvider, CanvasRepresentation, EditableAnnotation, FullPageViewport, Fullscreen, LocaleString, Manifest, ObservableElement, OpenSeadragonViewer, OpenSeadragonViewport, RangeNavigationProvider, Responsive, SingleTileSource, SizedViewport, StaticImageViewport, Viewport, functionOrMapChildren, htmlElementObserver, parseSelectorTarget, useAnnotationLists, useBemClassName, useCanvas, useManifest, withBemClass };
